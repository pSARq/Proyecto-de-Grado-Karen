...
export class DatosService {
  ...
  verifyToken(email: string): Observable<any> {
    ...
    return this.http.post(`${this.urlBase}/verify-token`, { email, currentToken }, { headers });
  }
  ...
  mostrarSolicitudes(): Observable<any> {
    ...
    return this.http.get(`${this.urlBase}/consultadeSolicitudes`, { headers });
  }
  ...
  tablaConsulta(): Observable<any> {
    ...
    // Realiza la petición GET a la URL correspondiente
    return this.http.get(`${this.urlBase}/tablaConsulta`, { headers });
  }
  ...
  registrarSolicitud(solicitud: any): Observable<any> {
    ...
    return this.http.post<any>(`${this.urlBase}/solicitudes`, solicitud, { headers }).pipe(
      // Resto del código...
    );
  }
  ...
  recuperarTodosID(filtro: any, filtro2: any) {
    const urlConFiltro = `${this.urlBase}/login/seleccionar.php?Filtro=${filtro}&Filtro2=${filtro2}`;
  ...
  recuperarSolicitudes(idEstudiante: string): Observable<any> {
    ...
    return this.http.get(`${this.urlBase}/consultaSolicitud/${idEstudiante}`, { headers });
  }
  ...
  eliminarSolicitud(idSolicitud: string): Observable<any> {
    ...
    const url = `${this.urlBase}/solicitudes/${idSolicitud}`;
  ...
  recuperarSolicitudesres(idEstudiante: string): Observable<any> {
    ...
    // Realiza la petición GET a la URL correspondiente, incluyendo el idEstudiante en la URL
    return this.http.get(`${this.urlBase}/consulta/${idEstudiante}`, { headers });
  }
  ...
  editarSolicitud(id: number, nuevaSolicitud: any, token: string): Observable<any> {
    ...
    const url = `${this.urlBase}/editarSolicitud/${id}`;
  ...
  recuperarEstado(id: any, consulta_id: any): Observable<any> {
    ...
    // Realiza la petición GET a la URL correspondiente, incluyendo el idEstudiante en la URL
    return this.http.get(`${this.urlBase}/consultaEstados/${id}/${consulta_id}`, { headers });
  }

  insertarConsulta(consultaData: any): Observable<any> {
    const url = `${this.urlBase}/insertarConsulta`;
  ...
  actualizarEstadoSolicitud(id: string, estado: string): Observable<any> {
    const url = `${this.urlBase}/actualizarEstadoSolicitud/${id}`;
  ...
  responderSolicitudes(id: any, nuevaSolicitud: any): Observable<any> {
    ...
    const url = `${this.urlBase}/insertRespuestas/${id}`;
  ...
  alta(registrouser: any) {
    return this.http.post(`${this.urlBase}/login/alta.php`, JSON.stringify(registrouser));
  }

  altaUser(reg: any) {
    return this.http.post(`${this.urlBase}/login/altauser.php`, JSON.stringify(reg));
  }

  baja(email: string) {
    const urlConFiltro = `${this.urlBase}/login/baja.php?email=${email}`;
    return this.http.get(urlConFiltro);
  }

  seleccionar(codigo: number) {
    return this.http.get(`${this.urlBase}/login/seleccionar.php?codigo=${codigo}`);
  }

  modificacion(articulo: any) {
    return this.http.post(`${this.urlBase}/login/modificacion.php`, JSON.stringify(articulo));
  }
}
